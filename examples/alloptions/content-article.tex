...
\begin{table}[H]
    \centering
    \captionabove{Die wichtigsten Befehle von \textbf{siunitx}}
    \begin{tabular}{|c|c|}
        \hline
        \textbackslash num\{Zahl\} & $\num{3.42}$ \\
        \textbackslash unit\{Einheit\} & $\si{\m}$ \\
        \textbackslash qty\{Zahl\}\{Einheit\} & $\SI{3.42}{\m}$ \\
        \hline
    \end{tabular}
    \label{tab:MUSTER_siunitx}
\end{table}

Wer den Quelltext der Tab \ref{tab:MUSTER_siunitx} vor sich stehen hat, wird feststellen, dass der Punkt durch ein Komma getauscht wurde. Des weiteren sorgt der letzte Befehl dafür, dass zwischen Zahl und Einheit kein Zeilenumbruch entstehen kann, da dies den Lesefluss, vorallem im Fließtext, stört.
